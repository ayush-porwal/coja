  ...
  return {
    ...
    list_files: tool({
      ...
      execute: async ({ ref, path }) => {
        ...
        const entries = (
          await lsTree(ctx.repo, oid, { recursive: true, ...(prefix ? { path: prefix } : {}) })
    ...
    git_log: tool({
      ...
      execute: async ({ path, maxCount }) => {
        ...
        const entries = await log(ctx.repo, {
...
async function fileDiff(ctx: ToolContext, file: string) {
  ...
  const paths =
    entry.previousPath !== undefined && entry.previousPath !== entry.path
      ...
      : [entry.path]
  const { patch, truncated } = await diff(ctx.repo, ctx.baseOid, ctx.headOid, { paths })
...
async function wholeDiff(ctx: ToolContext) {
  const full = await diff(ctx.repo, ctx.baseOid, ctx.headOid)
  ...
  if (!full.truncated) {
    ...
    const bare = await diff(ctx.repo, ctx.baseOid, ctx.headOid, { unified: 0, maxBytes: 32 * MiB })
